...
import CaseConverterFaq from './CaseConverterFaq';

const caseButtons = [
  { label: 'Sentence case', transform: caseConverter.toSentenceCase },
  { label: 'lower case', transform: caseConverter.toLowerCase },
  { label: 'UPPER CASE', transform: caseConverter.toUpperCase },
  { label: 'Capitalized Case', transform: caseConverter.toCapitalizedCase },
  { label: 'Title Case', transform: caseConverter.toTitleCase },
  { label: 'aLtErNaTiNg cAsE', transform: caseConverter.toAlternatingCase },
  { label: 'InVeRsE CaSe', transform: caseConverter.toInverseCase },
];
...
const CaseConverter: React.FC = () => {
  ...
  const { charCount, wordCount } = useMemo(() => {
    ...
    return {
      ...
      wordCount: trimmedText ? words.length : 0,
    };
  }, [text]);
    ...
    <>
      <div className="relative">
        <textarea
          ...
          aria-label="Text input area for case conversion"
        />
      </div>
