...
import { calculateGematria, GematriaSystem, systemLabels } from '../utils/gematria';
import GematriaCalculatorFaq from './GematriaCalculatorFaq';

const GematriaCalculator: React.FC = () => {
  const [text, setText] = useState<string>('Hello World');
  const [system, setSystem] = useState<GematriaSystem>('english_simple');
  ...
  const { totalValue, breakdown } = useMemo(() => calculateGematria(text, system), [text, system]);

  return (
    <>
      <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700">
        <label htmlFor="gematria-system" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Select Gematria System
        </label>
        <select
            id="gematria-system"
            value={system}
            onChange={(e) => setSystem(e.target.value as GematriaSystem)}
      ...
      <div className="relative">
        <textarea
          ...
          aria-label="Text input for Gematria calculation"
          dir="auto"
        />
      </div>
      ...
      <div className="mt-8 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 text-center">
        <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold mb-1">Total Gematria Value</p>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        ...
        {breakdown.length > 0 ? (
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Enter text above to see a breakdown of the calculation.
          </p>
        )}
      </div>
      
      <GematriaCalculatorFaq />
    </>
  );
};
